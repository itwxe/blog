...
上一篇 [Explain执行计划详解](https://itwxe.com/posts/4661f345.html) 中提到执行计划分析查询语句时 `possible_keys` 列中存在可以使用的索引，但是实际最后 `key` 列中却并没有使用索引，走的全表扫描，这是为啥捏？
